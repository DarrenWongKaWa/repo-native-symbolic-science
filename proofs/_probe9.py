#!/usr/bin/env python3
"""Probe v9: corrected T with 5-node divided difference F[z,z,z,x,y]; six-orbit identity."""
import mpmath as mp
mp.mp.dps = 60
I = mp.mpc(0, 1)
pi = mp.pi
BETA, GAMMA, MU = mp.mpf(5), mp.mpf('0.08'), mp.mpf(0)

def zplus(e): return mp.mpf(1)/2 + BETA*GAMMA/(2*pi) + I*BETA*(e - MU)/(2*pi)
def zminus(e): return mp.mpf(1)/2 + BETA*GAMMA/(2*pi) - I*BETA*(e - MU)/(2*pi)
def Phi(e):  return mp.mpf(1)/2 + (I/pi) * mp.digamma(zplus(e))
def Phis(e): return mp.mpf(1)/2 - (I/pi) * mp.digamma(zminus(e))
def F(e):    return Phi(e) + Phis(e)
def Fp(e):   return (I/pi)*mp.polygamma(1, zplus(e))*(I*BETA/(2*pi)) - (I/pi)*mp.polygamma(1, zminus(e))*(-I*BETA/(2*pi))
def Fpp(e):  return (I/pi)*mp.polygamma(2, zplus(e))*(I*BETA/(2*pi))**2 - (I/pi)*mp.polygamma(2, zminus(e))*(-I*BETA/(2*pi))**2
def Phi1(e): return (I/pi)*mp.polygamma(1, zplus(e))*(I*BETA/(2*pi))
def Phis1(e): return -(I/pi)*mp.polygamma(1, zminus(e))*(-I*BETA/(2*pi))
def Phi2(e): return (I/pi)*mp.polygamma(2, zplus(e))*(I*BETA/(2*pi))**2
def Phis2(e): return -(I/pi)*mp.polygamma(2, zminus(e))*(-I*BETA/(2*pi))**2

def H1(f, u, v): return (f(v) - f(u)) / (v - u)
def R1(wv, xx, yy):
    num = (mp.mpf(1)/2)*(Phi(yy) + Phis(yy) - Phi(xx) - Phis(xx)) + I*GAMMA*(H1(Phis, xx, yy + wv) + H1(Phi, xx - wv, yy))
    return num / (wv + yy - xx + 2*I*GAMMA)
def w2(f):
    h = mp.mpf('1e-12')
    return (f(h) - 2*f(mp.mpf(0)) + f(-h)) / (2*h*h)
def M(x, y): return w2(lambda wv: R1(wv, x, y))

# 5-node divided difference F[z,z,z,x,y] via recursion (z repeated 3x)
def dd(f, nodes):
    """generic divided difference (nodes list, possibly repeated)."""
    n = len(nodes)
    if n == 1: return f(nodes[0])
    if n == 2:
        u, v = nodes
        if u == v: return Fp(u)
        return (f(v) - f(u)) / (v - u)
    # recursion: f[nodes] = (f[nodes[1:]] - f[nodes[:-1]]) / (last - first)
    a, b = nodes[0], nodes[-1]
    if a == b:
        return None  # handle repeated endpoints below
    return (dd(f, nodes[1:]) - dd(f, nodes[:-1])) / (b - a)

def dd5(z, x, y):
    """F[z,z,z,x,y]: recursion with explicit coincident handling."""
    # f[z,z,z,x,y] = (f[z,z,x,y] - f[z,z,z,x])/(y - z)
    # f[z,z,x,y] = (f[z,x,y] - f[z,z,x])/(y - z)
    # f[z,x,y] = (f[x,y] - f[z,x])/(y - z)
    # f[z,z,x] = (f[z,x] - f[z,z])/(x - z); f[z,z] = Fp(z)
    # f[z,z,z,x] = (f[z,z,x] - f[z,z,z])/(x - z); f[z,z,z] = Fpp(z)/2
    f = F
    fxy = (f(y) - f(x)) / (y - x)
    fzx = (f(x) - f(z)) / (x - z)
    fzz = Fp(z)
    fzzx = (fzx - fzz) / (x - z)
    fzxy = (fxy - fzx) / (y - z)
    fzzxy = (fzxy - fzzx) / (y - z)
    fzzz = Fpp(z) / 2
    fzzzx = (fzzx - fzzz) / (x - z)
    fzzzxy = (fzzxy - fzzzx) / (y - z)
    return fzzzxy

def T(a, b, c):
    """FROZEN contract: T_abc = (M_cb - M_ac + i Gamma F[E_c,E_c,E_c,E_a,E_b])/(E_b - E_a + 2 i Gamma)"""
    return (M(c, b) - M(a, c) + I*GAMMA*dd5(c, a, b)) / (b - a + 2*I*GAMMA)

# confluence anchor: T(x,x,x) -> F4/48
def F4(e):
    zp = I*BETA/(2*pi); zm = -I*BETA/(2*pi)
    return (I/pi)*mp.polygamma(4, zplus(e))*zp**4 - (I/pi)*mp.polygamma(4, zminus(e))*zm**4

x0 = mp.mpf('0.4')
print("confluence: T(x,x+eps,x-eps) vs F4/48:")
for eps in (mp.mpf('1e-3'), mp.mpf('1e-6'), mp.mpf('1e-9')):
    t = T(x0, x0 + eps, x0 - eps)
    print("  eps=%s  diff=%s" % (mp.nstr(eps, 4), mp.nstr(abs(t - F4(x0)/48), 25)))

ea, eb, ec = mp.mpf('-0.5'), mp.mpf('0.3'), mp.mpf('1.4')
def Delta(e1, e2): return e1 - e2
def K(a, b, c):
    Dab, Dac, Dbc = Delta(a,b), Delta(a,c), Delta(b,c)
    return I*(Dab*(Dac - Dbc)*M(a, b) - Dab*Dac*Dbc*T(a, b, c))
def D3(a, b, c):
    Dab, Dac, Dbc = Delta(a,b), Delta(a,c), Delta(b,c)
    t1 = -(mp.mpf(1)/Dac + mp.mpf(1)/Dbc) * (8*GAMMA*Dab/(Dab + 2*I*GAMMA)) * (mp.mpf(1)/2)*Phis1(a)
    t2 = (2*GAMMA*Dab/(Dab + 2*I*GAMMA)) * (mp.mpf(1)/2)*Phis2(a)
    return mp.re(t1 + t2)

import itertools
SK = mp.mpc(0); SD = mp.mpc(0)
for p in itertools.permutations((ea, eb, ec)):
    SK += K(*p); SD += D3(*p)
print()
print("sumK  :", mp.nstr(SK, 40))
print("sumD3 :", mp.nstr(SD, 40))
print("total :", mp.nstr(SK + SD, 35))
print("Im sumK:", mp.nstr(mp.im(SK), 30))
